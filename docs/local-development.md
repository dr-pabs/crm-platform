...
cd src/frontend/customer-portal && npm install
```
...
Both portals proxy `/api` to `http://localhost:5000` (via their `vite.config.ts`). Ensure the relevant backend services are running.

### Run frontend tests

```bash
cd src/frontend/staff-portal && npm test
cd src/frontend/customer-portal && npm test
```
