...
- Azure Static Web Apps (2 — staff portal + customer portal)
...
- 2 × Azure Static Web Apps (staff portal + customer portal)
...
| `Entra--CustomerClientId` | App registration client ID (customer portal) |
| `Acs--ConnectionString` | Azure Communication Services connection string |

### 3.2 Configure Entra ID app registrations

Two app registrations are required in the **customer's Entra tenant**:
...
**Customer portal (Entra External ID / CIAM):**
1. Create External ID tenant (if not existing)
2. Create app registration: `CRM Customer Portal - <ClientName>`
3. Add redirect URI: `https://<customer-portal-url>/auth/callback`
4. Record the Client ID and store in Key Vault as `Entra--CustomerClientId`

The customer's IT admin must approve consent for both app registrations.
