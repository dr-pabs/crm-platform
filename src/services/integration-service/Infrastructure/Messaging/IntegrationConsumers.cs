...
file sealed record TenantProvisionedMessage(Guid TenantId, string Slug, string AdminEmail);
file sealed record TenantSuspendedMessage(Guid TenantId, string Reason);
file sealed record LeadAssignedMessage(Guid TenantId, Guid LeadId, Guid AssignedToUserId, string LeadName);
file sealed record OpportunityWonMessage(Guid TenantId, Guid OpportunityId, string Title, decimal Amount);
file sealed record CaseCreatedMessage(Guid TenantId, Guid CaseId, string CaseNumber, string Title);
file sealed record CaseResolvedMessage(Guid TenantId, Guid CaseId, string CaseNumber);
    ...
    : BaseServiceBusConsumer<LeadAssignedMessage>(serviceBusClient, idempotencyStore, options, logger)
{
    ...
    {
        await EnqueueOutboundJobsAsync(tenantId, "lead.assigned",
    ...
    : BaseServiceBusConsumer<OpportunityWonMessage>(serviceBusClient, idempotencyStore, options, logger)
{
    ...
    {
        await EnqueueOutboundJobsAsync(tenantId, "opportunity.won",
            System.Text.Json.JsonSerializer.Serialize(msg), scopeFactory, logger, ct);
        return true;
    }
}
    ...
    : BaseServiceBusConsumer<CaseCreatedMessage>(serviceBusClient, idempotencyStore, options, logger)
{
    ...
    {
        await EnqueueOutboundJobsAsync(tenantId, "case.created",
            System.Text.Json.JsonSerializer.Serialize(msg), scopeFactory, logger, ct);
        return true;
    }
}
    ...
    : BaseServiceBusConsumer<CaseResolvedMessage>(serviceBusClient, idempotencyStore, options, logger)
{
    ...
    {
        await EnqueueOutboundJobsAsync(tenantId, "case.resolved",
            System.Text.Json.JsonSerializer.Serialize(msg), scopeFactory, logger, ct);
        return true;
    }
}
