...
public sealed class AcsOptions
{
    ...
    public string NotificationHubNamespace   { get; init; } = string.Empty;
    ...
    : INotificationSender
{
    ...
    private async Task<string> SendSmsAsync(NotificationRecord record, CancellationToken ct)
    {
        ...
        var msgId = response.Value.FirstOrDefault()?.MessageId ?? Guid.NewGuid().ToString();
    ...
    {
        var client = new NotificationHubsClient(
            _opts.NotificationHubNamespace,
            _opts.NotificationHubName);

        // RecipientAddress is the device registration tag (userId:<guid>)
        var notification = new FcmV1Notification(
            $$"""
            {
              "message": {
                "notification": {
        ...
        var result = await client.SendNotificationAsync(
            notification,
            tagExpression: record.RecipientAddress,
            cancellationToken: ct);

        var trackingId = result.Value.TrackingId ?? Guid.NewGuid().ToString();
