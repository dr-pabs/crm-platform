    ...
    public static IEndpointRouteBuilder MapNotificationEndpoints(this IEndpointRouteBuilder app)
    {
            ...
            ITenantContext ctx) =>
        {
            if (!Enum.TryParse<NotificationChannel>(req.Channel, true, out var channel))
                return Results.Problem("Invalid channel.", statusCode: 400);
            ...
            return result.IsSuccess
                ? Results.Created($"/notification-templates/{result.Value}", new CreatedResponse(result.Value))
                : result.ToHttpResult();
        });
            ...
            SendNotificationHandler handler) =>
        {
            ...
            return result.IsSuccess
                ? Results.Ok(new CreatedResponse(result.Value))
                : result.ToHttpResult();
        });
